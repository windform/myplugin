...
$.fn.filter=function(){
	...
	$.ajax({
		...
		success:function(result){
			...
			// 样式行为控制
			$('.span_filter').css({
				width:$('.row').width()-$('.span_more_filter').width()-$('.span_label').width()*2,
			...
			$('.ul_filter li').on('click',function(){
				...
				if($(this).find('i').hasClass('yin')){
					$('.select-text').append('<span class="span-select">'+value+'<i class="closeX"></i></span>');
			...
			$('a.confirm').click(function(){
				...
				$('.select-text').append('<span class="span-select">'+arrToStr+'<i class="closeX"></i></span>');
			...
			$('.select-text').on('click','.span-select',function(str){
				...
				//alert($.type(value));
				//alert(strBase);
				$("div.bor_top_line:contains('"+value+"')").css('display','block');
				//$("div.bor_top_line:contains('"+strBase+"')").css('display','block');
				$(this).remove();
			})
			// 更多筛选单击显示、隐藏
			$('.more-filter-toggle').click(function(){
				$('.bor_top_line,.select-text').toggle();
			})
			// 单击更多显示更多筛选条件
			...
			$('.more-select').click(function(){
				...
				switch(html){
					case (html='更多&nbsp;<span class="caret"></span>'):
						$('.more-select').html('更多&nbsp;<span class="caret"></span>');
						$('.more-select').parent().parent().css('height',41);
			...
			$('.multiple-select').click(function(){
						$('.multiple-select').parent().parent().css('height',41);
						$('.multiple-select').parent().prev().find('li i').removeClass('xian').addClass('yin');
						$('.multiple-select').parent().css('display','block').next().css('display','none');
						$('.multiple-condition').addClass('yin');
						$(this).parent().parent().css('height','auto');
